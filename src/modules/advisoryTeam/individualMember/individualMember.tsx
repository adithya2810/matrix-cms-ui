...
const IndividualMember: FC<propsType> = (props) => {
  ...
  return (
    <div className="team_detail py-56 px-12 sm:py-0 sm:px-0">
      ...
      {props.advisoryDetail.map((data, index) => (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-4 sm:gap-0 md:gap-0 pt-28">
          <div className="hide-mobile hide-tab pl-16 justify-center bg-secondary flex flex-col">
            <div className="flex justify-between">
              <h3 className="text-7xl leading-tight text-accent font-normal">Latest <br />content<br /> from<br /> {data.name.replace(' ', '\n')}</h3>
              <Link href="#" passHref>
          ...
          {/* Mobile Section */}
          <h3 className="hidden sm:block md:block lg:block text-2xl leading-tight text-accent font-normal px-7">Latest content from {data.name}</h3>
          {/* Mobile Section */}
          ...
          <div className="col-span-2 pl-32 sm:px-7 sm:py-8 md:px-7 md:py-8 lg:px-7 lg:py-8">
            {data.blogs?.map((e, index) => (
              <div className="laptop:flex mb-32 sm:mb-0 lg:mb-10 justify-center" key={index}>
                <div className="self-start relative flex-shrink-0 w-90 sm:w-full sm:ml-3" onClick={_ => push(`/blogs/${e.slug}`)}>
                  <img
                    className="image-shadow"
                    width="100%"
                    style={!props.deviceType?.mobile ? { height: 178, width: 220 } : { height: 'auto', width: '100%' }}
                    src={!props.deviceType?.mobile ? e.cover_desktop : e.cover_image_mobile}
                    alt="image"
                  />
                ...
                >
                  <div className="caption text-accent-dark mb-4 sm:mb-2">{e.readtime.toUpperCase()} READ</div>
                  <div className="sub-h1">{e.name}</div>
                  <div className="flex mt-6 mb-6 items-center">
                    ...
                    <div className="flex pl-5 sm:pl-0">
                      ...
                      />
                      <div>
                        <p className="sub-h2 text-accent-dark">Author name</p>
