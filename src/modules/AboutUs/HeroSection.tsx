  ...
  return (
    <>
      <Carousel autoPlay showThumbs={false} showArrows={false} showStatus={false} infiniteLoop={true} interval={6000} stopOnHover={false} transitionTime={1000} centerMode={false} onChange={index => setSlideCount(index + 1)}>
        {[1, 2, 3, 4, 5].map(ele => (
          <div key={ele} className={`overlay bg-cover relative section flex items-center`} style={{ backgroundImage: `url(${data(deviceType.mobile).imgPath})`, height: deviceType.mobile ? 823 : 900, zIndex: 0 }}>
            <div className="text-white relative -top-24" style={{ textAlign: 'start' }}>
              ...
              <h5 className='mb-10 ma-slider-fix'>{data(deviceType.mobile).text}</h5>
