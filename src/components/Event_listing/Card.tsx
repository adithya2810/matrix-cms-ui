...
const Card: FC<propsType> = (props) => {
  ...
  const getImage = (data) => {
    if (data.hasOwnProperty('image') && data.image) {
      if (data.image.hasOwnProperty('formats') && data.image.formats.hasOwnProperty('thumbnail')) {
        return data.image.formats.thumbnail.url;
      } else {
        return data.image.url;
      }
    } else {
      return data.cover_image_url || '/icons/matrixLogo.svg';
    }
  };
  ...
  return (
    ...
    <div className="laptopCustom:flex">
      ...
      >
        ...
        {/* <p className="font-medium" style={{ color: '#000000', fontSize: 13 }} dangerouslySetInnerHTML={{ __html: props.description.replace(/<[^>]+>/g, '').slice(0, 200) }}></p> */}
