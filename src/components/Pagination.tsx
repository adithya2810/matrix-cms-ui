...
const getPagination = (currentPage, totalPages, push) => {
  ...
  return rangeWithDots.reduce((acc, n) => {
    if (n < 10) n = '0' + n;
    acc.push(<div onClick={_ => +n && push(`/blogs?page=${n}`)} className={`${+n ? 'cursor-pointer' : ''} body1 mx-2 px-4 sm:mx-1 sm:px-2 ${currentPage === +n ? 'text-white bg-accent-dark py-1' : +n ? 'bg-grey-dark text-accent-dark' : ''} py-1 hover:opacity-80`}>{n}</div>)
    return acc
  }, []);
}
