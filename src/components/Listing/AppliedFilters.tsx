...
const getFilters = (query, pageType) => {
  let filters = [];
  if (query.tags) filters.push(query.tags);
  ...
  return filters.map((f) => (
    <div className="border-2 border-accent flex items-center sub-h2 laptop:py-2 laptop:px-4 sm:py-1 sm:px-2">
      ...
      <span className="cursor-pointer" onClick={() => window.location.href = `${pageType ? '/' + pageType : ''}/news`}>
        <CloseCross />
      </span>
    </div>
  ));
  ...
  return (
    <div className="section">
      <div className="laptop:flex laptop:items-center mt-10 mb-6 sm:mt-5 sm:mb-3">
        ...
        {(query.tags || query.search) && <>
          <div className="justify-self-center flex gap-4 laptop:mr-8 sm:overflow-x-scroll">
            {getFilters(query, pageType)}
          </div>
          <div className="sub-h2 text-accent underline sm:hidden cursor-pointer" onClick={() => window.location.href = `${pageType ? '/' + pageType : ''}/news`}>Clear All</div>
        </>}
      </div>
